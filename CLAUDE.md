...
de Coplay (`mcp__coplay-mcp__*`).

| Tarea | Cómo |
|---|---|
| Ejecutar pruebas (o una sola) | Skill `unity-coding-skills:run-tests` → `run_unity_tests`, filtrando por assembly o nombre. Nunca invocar la herramienta ad hoc. **Hoy esa herramienta no está conectada — ver la nota de abajo.** |
| Errores de compilación | `mcp__coplay-mcp__check_compile_errors` / `mcp__coplay-mcp__get_unity_logs`, en vez de adivinar. |
| Escenas, prefabs, assets | Herramientas MCP de Coplay + skill `edit-scene`; no manipular estado del Editor a mano. |
| Build de entrega | Editor → Build Profiles → Windows x64, salida a carpeta portable. |

Si el puente Coplay no conecta, no hay forma de tocar escenas: reiniciar el Editor y el puente
antes de empezar.
...
Test Runner del Editor, y hay que **decirlo**, nunca dar por hecho que la suite pasó. Instalar
ese servidor es lo que desbloquea el flujo test-first del que depende todo `SPEC.md`.
...
nombre (CT-10). Cada commit se asocia a su tarjeta del Kanban (RNF-17).
